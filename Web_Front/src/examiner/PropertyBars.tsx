  ...
  return (
    <>
      {BAR_PROPS.map(prop => {
        ...
        const isSeconds = prop.endsWith('_seconds');
        const unit = isSeconds ? ' s' : (prop.endsWith('_hz') || prop === 'complexity') ? ' Hz' : '';
        // Attack & length are times — also show them in samples.
        const inSamples = isSeconds ? `${Math.round(val * sr).toLocaleString()} smp` : null;
        return (
          <div key={prop} style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
              <span>{prop}</span>
              <span style={{ color: '#FCD34D' }}>{val.toFixed(2)}{unit}{inSamples ? ` · ${inSamples}` : ''}</span>
            </div>
