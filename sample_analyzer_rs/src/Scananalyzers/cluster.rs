...
pub fn cluster_samples(results: &mut [Peak], k: usize) {
    let idx_hits: Vec<usize> = (0..results.len()).filter(|&i| results[i].classification.group != "Loops/Patterns").collect();
    let idx_loops: Vec<usize> = (0..results.len()).filter(|&i| results[i].classification.group == "Loops/Patterns").collect();
